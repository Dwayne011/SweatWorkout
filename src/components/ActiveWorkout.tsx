...
export default function ActiveWorkout({
  ...
  return (
    <div className="bg-white/70 dark:bg-zinc-950/75 backdrop-blur-2xl rounded-2xl border border-gray-200/80 dark:border-white/10 shadow-2xl overflow-hidden relative z-10 w-full max-w-full">
      ...
      <div className={`border-b px-4 py-5 shrink-0 flex flex-col items-stretch gap-4 text-gray-900 dark:text-gray-100 transition-all duration-300 ${
        restTimerTarget 
        ...
        <div className="flex items-center justify-between min-w-0 w-full">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 mb-1.5">
              <h3 className="font-black text-gray-900 dark:text-gray-100 text-lg md:text-xl tracking-tight truncate whitespace-nowrap">
                {session.name}
              </h3>
              <span className="m3-livedot shrink-0" />
            </div>
            {restTimerTarget ? (
              <p className="text-xs font-mono font-bold flex flex-wrap items-center gap-1.5">
              ...
              <p className="text-xs text-indigo-600 dark:text-indigo-300/80 font-mono font-bold flex items-center gap-1.5">
                <span>ACTIVE SESSION TRACKED</span>
                <span>•</span>
                <b><ActiveWorkoutDuration startTime={session.startTime} /></b>
              </p>
            )}
          </div>
        </div>

        {/* Squiggly flow wave divider in place of plain divider */}
        <svg className="m3-wave" viewBox="0 0 320 22" preserveAspectRatio="none">
          <path d="M0 11 Q 10 1 20 11 T 40 11 T 60 11 T 80 11 T 100 11 T 120 11 T 140 11 T 160 11 T 180 11 T 200 11 T 220 11 T 240 11 T 260 11 T 280 11 T 300 11 T 320 11" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round"/>
        </svg>

        <div className="flex items-center gap-3 w-full">
          <button
            onClick={() => { if (typeof navigator !== 'undefined' && navigator.vibrate) navigator.vibrate(10); setShowDiscardConfirm(true); }}
            className="flex-1 py-3 bg-rose-500 hover:bg-rose-400 text-white text-xs font-black rounded-full shadow-lg flex items-center justify-center space-x-1.5 transition-all cursor-pointer whitespace-nowrap"
          >
          ...
          <button
            ...
            className="flex-[1.3] py-3 bg-emerald-500 hover:bg-emerald-400 text-white text-xs font-black rounded-full shadow-lg flex items-center justify-center space-x-1.5 transition-all cursor-pointer whitespace-nowrap"
          >
      ...
      {showDiscardConfirm && createPortal(
        <div className="fixed inset-0 bg-gray-900/40 dark:bg-black/80 z-[10000] flex items-center justify-center p-4 backdrop-blur-md">
          <motion.div
            initial={{ scale: 0.95, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            className="bg-white dark:bg-zinc-950 border border-gray-200 dark:border-white/10 rounded-[28px] w-full max-w-sm overflow-hidden p-6 shadow-2xl space-y-4 shadow-black/10 dark:shadow-black/50"
          >
            <div className="text-center space-y-3">
              <div className="mx-auto w-16 h-16 text-rose-500 flex items-center justify-center relative">
                {/* Organic cookie blob shape background */}
                <svg className="absolute inset-0 w-full h-full text-rose-500/10 fill-current" viewBox="0 0 100 100">
                  <path d="M 50.00 5.00 C 60.80 5.00, 51.08 8.88, 60.35 11.36 C 69.63 13.85, 63.15 5.63, 72.50 11.03 C 81.85 16.43, 71.50 14.93, 78.28 21.72 C 85.07 28.50, 83.57 18.15, 88.97 27.50 C 94.37 36.85, 86.15 30.37, 88.64 39.65 C 91.12 48.92, 95.00 39.20, 95.00 50.00 C 95.00 60.80, 91.12 51.08, 88.64 60.35 C 86.15 69.63, 94.37 63.15, 88.97 72.50 C 83.57 81.85, 85.07 71.50, 78.28 78.28 C 71.50 85.07, 81.85 83.57, 72.50 88.97 C 63.15 94.37, 69.63 86.15, 60.35 88.64 C 51.08 91.12, 60.80 95.00, 50.00 95.00 C 39.20 95.00, 48.92 91.12, 39.65 88.64 C 30.37 86.15, 36.85 94.37, 27.50 88.97 C 18.15 83.57, 28.50 85.07, 21.72 78.28 C 14.93 71.50, 16.43 81.85, 11.03 72.50 C 5.63 63.15, 13.85 69.63, 11.36 60.35 C 8.88 51.08, 5.00 60.80, 5.00 50.00 C 5.00 39.20, 8.88 48.92, 11.36 39.65 C 13.85 30.37, 5.63 36.85, 11.03 27.50 C 16.43 18.15, 14.93 28.50, 21.72 21.72 C 28.50 14.93, 18.15 16.43, 27.50 11.03 C 36.85 5.63, 30.37 13.85, 39.65 11.36 C 48.92 8.88, 39.20 5.00, 50.00 5.00 Z" />
                </svg>
                <AlertTriangle className="w-8 h-8 text-rose-500 relative z-10 animate-pulse" />
              </div>
              <h3 className="text-lg font-black text-gray-900 dark:text-gray-100 tracking-tight">
                Discard active workout?
              </h3>
              <p className="text-xs text-gray-600 dark:text-slate-400 leading-relaxed px-1">
                This clears your currently tracked exercises and can't be undone.
              </p>
            </div>

            <div className="flex gap-1 bg-slate-100 dark:bg-[#2f273f] p-1.5 rounded-full mt-4">
              <button
                onClick={() => {
                  setShowDiscardConfirm(false);
                  onDiscard();
                }}
                className="flex-1 border-none py-3.5 rounded-full font-extrabold text-xs flex items-center justify-center gap-1.5 bg-transparent hover:bg-rose-500/10 text-rose-500 transition-colors"
              >
              ...
              <button
                ...
                className="flex-none w-14 border-none py-3.5 rounded-[18px] bg-slate-200 dark:bg-[#3a3149] text-purple-505 flex items-center justify-center transition-colors"
              >
                <Pause className="w-4 h-4" />
              </button>
              <button
                onClick={() => setShowDiscardConfirm(false)}
                className="flex-1 border-none py-3.5 rounded-full font-extrabold text-xs flex items-center justify-center gap-1 text-emerald-500 bg-transparent hover:bg-[#1baf6e]/10 transition-colors"
              >
                <span>Continue</span>
              </button>
            </div>
